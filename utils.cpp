...
void recognition(Matrix * vectors,int *n, int *m){ 
  ...
  bool * stables = new bool[*m]; // stables[i] = true if v[i] is stable
  bool * double_c = new bool[*m]; // double_c[i] = true if v[i] is a double cycle
  ...
  showMany(w,n,m);
  cout << "\n--INITIALS VECTORS--\n\n";
  showMany(vectors,stables,double_c,n,m);
...
void showMany(Matrix * vectors, bool * stables, bool * double_c, int * n, int * m){
  ...
  for(int l = 0; l < (*m); l++){
    if(stables[l])
      cout <<right<<setw(s)<<"\033[0;32m"<<"STABLE ";
    else if(double_c[l])
      cout <<right<<setw(s)<<"\033[0;36m"<<"DOUBLE ";
    else
      cout <<right<<setw(s)<<"\033[0;31m"<<"NOTHING";
  }
  cout << endl;
}
